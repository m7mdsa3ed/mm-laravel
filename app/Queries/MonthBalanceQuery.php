        ...
        return DB::table('transactions')
            ...
            ->select(
                ...array_map(fn($sql) => DB::raw($sql), [
                    ...
                    'COALESCE(SUM(CASE WHEN action = 1 AND action_type = 7 THEN amount ELSE 0 END), 0) AS in_adjust_amount',
                    'COALESCE(SUM(CASE WHEN action = 2 AND action_type in (1,2,7) THEN amount ELSE 0 END), 0) - sum(amount * if (action_type = 7, 1, 0)) AS out_amount',
                    ...
                    'COALESCE(SUM(CASE WHEN action = 2 AND action_type = 7 THEN amount ELSE 0 END), 0) AS out_adjust_amount',
